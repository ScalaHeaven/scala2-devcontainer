# Scala 2 Devcontainer

This repository is a ready-to-open Scala 2 development workspace for VS Code Dev
...
```bash
docker build -t scala2-devcontainer .
docker run --rm scala2-devcontainer
```
...
`scala` and `scalac` pinned to Scala 2.13.18 so you can check the runner and
compiler directly.
...
This is the example Scala 2 application. It defines a standard `main` method on
`object Main`:
...
The main class is named `Main`, which is why the sbt assembly and VS Code launch
configuration both reference `Main`.
...
This defines the VS Code debug target named `Main`. It uses the Scala debug
...
- Coursier
- Scala compiler and runner, Scala CLI, Metals MCP, and an sbt wrapper
