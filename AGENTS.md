...
- a Scala 2 sbt project with a small example application
...
- `src/main/scala/Main.scala`: example Scala 2 application with the `Main`
  main class.
...
- `Dockerfile`: production multi-stage build that runs `sbt assembly` and
  launches `target/scala-2.13/app.jar`.
...
- Follow idiomatic Scala 2 syntax and keep examples simple unless the task asks
...
- compiling and running a Scala 2 app with sbt
